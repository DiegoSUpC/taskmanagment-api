...
import com.org.exception.BussinesRuleException;
...
public class TaskService {
    ...
    public TaskResponse create(TaskRequest request) {
        ...
        if (activeCount >= MAX_ACTIVE_TASKS) {
            throw new BussinesRuleException("Solo se puede asignar una tarea");
        }
    ...
    public TaskResponse update(Long id, TaskRequest request) {
        ...
        if (task.getStatus() != TaskStatus.PENDING) {
            throw new BussinesRuleException("Solo se puede reasignar una tarea en estado PENDING");
        }
        ...
        if (!task.getDeveloper().getId().equals(newDeveloper.getId())) {
            ...
            if (activeCount >= MAX_ACTIVE_TASKS) {
                throw new BussinesRuleException("El nuevo developer tiene tareas activas al límite");
            }
    ...
    public TaskResponse changeStatus(Long id, TaskStatus newStatus) {
        ...
        if (newStatus == TaskStatus.COMPLETED && current != TaskStatus.IN_PROGRESS) {
            throw new BussinesRuleException("Solo se puede completar una tarea que estuvo en progreso");
        }

        if (newStatus == TaskStatus.IN_PROGRESS && current != TaskStatus.PENDING) {
            throw new BussinesRuleException("Solo se puede iniciar una tarea que esté pendiente");
        }
    ...
    public Page<TaskResponse> findByDateRange(LocalDate start, LocalDate end, Pageable pageable) {
        ...
        if (start.isAfter(end)) {
            throw new BussinesRuleException("La fecha de inicio no puede ser posterior a la fecha de fin");
        }
